...
class Warframe(commands.Cog):
    ...
    async def news(self, ctx):
        ...
        news_embed = discord.Embed(
            color=0x0081A2, title="Warframe News", timestamp=ctx.message.created_at
        )
            ...
            async with session.get("https://api.warframestat.us/pc/news") as request:
                ...
                for article in reversed(news):
                    ...
                    date = datetime.strptime(
                        article["date"], "%Y-%m-%dT%H:%M:%S.%fZ"
                    ).strftime("%#d %B %Y")
                    news_embed.add_field(
                        name=f'{date} | {article["eta"]}',
                        value=f"[{title}]({url})",
                        inline=False,
                    )
    ...
    async def nightwave(self, ctx):
        ...
        nw_embed = discord.Embed(
            color=0x8A0030,
            title="This week's nightwave",
            ...
            async with session.get(
                ...
                for challenge in nw["activeChallenges"]:
                    ...
                    start = datetime.strptime(
                        challenge["activation"], "%Y-%m-%dT%H:%M:%S.%fZ"
                    )
                    end = datetime.strptime(
                        challenge["expiry"], "%Y-%m-%dT%H:%M:%S.%fZ"
                    )
                    remaining = abs(end.date() - start.date()).days
                    if (
                        challenge["reputation"] == 1000
                    ):  # Because if challenge["isDaily"] doesn't fucking work
                        nw_embed.add_field(
                            name=f"[Daily] {challenge_title}",
                            value=f"{challenge_desc}\n**Time remaining**: {remaining} Days",
                            inline=False,
                        )
                    if challenge["reputation"] == 4500:
                        nw_embed.add_field(
                            name=f"[Weekly] {challenge_title}",
                            value=f"{challenge_desc}\n**Time remaining**: {remaining} Days",
                            inline=False,
                        )
                    if challenge["isElite"]:
                        nw_embed.add_field(
                            name=f"[Elite Weekly] {challenge_title}",
                            value=f"{challenge_desc}\n**Time remaining**: {remaining} Days",
                            inline=False,
                        )
            ...
            async with session.get(
                ...
                if not baro["inventory"]:
                    baro_embed.add_field(
                        name="Arrives in", value=baro["startString"], inline=False
                    )
                    baro_embed.add_field(
                        name="Relay:", value=baro["location"], inline=False
                    )
                    baro_embed.add_field(
                        name="Inventory",
                        value="Baro Ki'Teer is yet to arrive.",
                        inline=False,
                    )
                else:
                    baro_embed.add_field(
                    ...
                    for item in baro["inventory"]:
                        ...
                        baro_embed.add_field(
                            name=item_name,
                            value=f"{ducats} <:Ducats:573969761109803056> +\n"
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                ...
                async with session.get(
                    ...
                    cetus_embed.add_field(
                        name="Current Bounties", value="\u200b", inline=False
                    )
                    for jobs in ostron_syndicate["jobs"]:
                        ...
                        cetus_embed.add_field(
                            name=f"{job_name} | levels {levels[0]} - {levels[1]}",
                            value=f"```asciidoc\nRewards :: {rewards_str}\n```",
                            inline=False,
                        )
    ...
    async def vallis(self, ctx):
        ...
        vallis_embed = discord.Embed(
            title="Orb Vallis state",
    ...
    async def cambion(self, ctx):
        ...
        randfunne = random.randrange(len(self.funne))
        cambion_embed = discord.Embed(
            title="Cambion Drift bounties",
    ...
    async def sorties(self, ctx):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                mission_no = 0
                sortie_embed = discord.Embed(
                    title=f"Today's Sortie | {faction}",
                    color=discord.Colour.purple(),
                    timestamp=ctx.message.created_at,
                )
                sortie_embed.add_field(
                    name="Time left", value=sortie["eta"], inline=False
                )
                for mission in sortie["variants"]:
                    ...
                    sortie_embed.add_field(
                        name=f"Mission #{mission_no + 1} | {node}",
            ...
            async with session.get("https://api.warframestat.us/pc/rivens") as request:
                ...
                for query in rivens:
                    for wep in rivens[query]:
                        if (
                            fuzz.token_sort_ratio(name.split()[:2], wep.split()[:2])
                            == 100
                        ):  # Only compare first 2 words so people don't have to type "Veiled Archgun Riven Mod" and could just type "Veiled Archgun"
                            wepinfo = rivens[query][wep]
                            if (
                                "veiled" in name.lower()
                            ):  # Veiled rivens don't have rerolled info so no need to parse unrolled info
                                embed = discord.Embed(
                                    title=f'Riven info for: {wepinfo["unrolled"]["compatibility"]}',
                                    color=0xA45EE5,
                                    timestamp=ctx.message.created_at,
                                )
                                embed.add_field(
                                ...
                                embed.add_field(
                                    ...
                                    f'\nMedian Price: {wepinfo["rerolled"]["median"]} <:Platinum:573969761919303720>',
                                    inline=False,
                                )
                                if (
